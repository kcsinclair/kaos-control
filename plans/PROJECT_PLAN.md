...
**Current stage:** Agent runtime hardening — model selection wired through to `--model`; per-agent `timeout_minutes` (0 disables) with new `killed-timeout` status separate from user-initiated `killed`; `blocked` status + `* → blocked` / `blocked → draft` transitions so agents can self-block on missing input; stream-json output parsed live into the existing progress panel; per-run log files persisted at `<data>/<project>/runs/<run_id>.log` and viewable from the UI.
