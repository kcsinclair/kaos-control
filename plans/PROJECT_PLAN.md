...
**Current stage:** Role vocabulary migration complete — analyst (think) / backend-developer + frontend-developer + test-developer (make) / qa (verify); six new agent definitions; new `defect` artifact type and `lifecycle/defects/` stage; transition matrix + required_plans gate updated; spec aligned. Ready to start exercising the workflow with real agent runs.
