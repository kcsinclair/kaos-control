...
export ANTHROPIC_BASE_URL=http://leia.tek42.io:11434
claude --model nutboy02/Qwen3.6-35B-A3B-Claude-4.7-Opus-abliterated-uncenfull
...
The driver would accept a `--model` flag (or equivalent config field) pointing to an Ollama model tag (e.g. `gemma4:26b-mlx` or `nutboy02/Qwen3.6-35B-A3B-Claude-4.7-Opus-abliterated-uncenfull`), and set the auth token to the literal string `"ollama"` as required by the Ollama Claude-compatible shim. The base URL would point to the local or network Ollama instance (e.g. `http://localhost:11434` or a LAN host).
