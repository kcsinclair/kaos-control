...
const option = computed(() => {
  ...
  const series = models.map((model) => {
    const byBucket = Object.fromEntries(
      props.seriesByModel[model].map((p) => [p.bucket_start, p.mean_output_tokens_per_second])
    )
    return {
      name: model,
      type: 'line',
  ...
  return {
    ...
    yAxis: { type: 'value', name: 'tokens/s', nameLocation: 'middle', nameGap: 45, axisLabel: { fontSize: 11 } },
    series,
  }
})
...
<template>
  <div class="chart-wrap">
    <h3 class="chart-title">Output tokens/s by model</h3>
    <div ref="chartEl" class="chart-canvas" role="img" aria-label="Output tokens per second line chart" />
  </div>
</template>
