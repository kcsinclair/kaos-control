...
const option = computed(() => {
  ...
  const series = models.map((model) => {
    const byBucket = Object.fromEntries(
      props.seriesByModel[model].map((p) => [p.bucket_start, p.mean_ttft_ms])
    )
    return {
      name: model,
      type: 'line',
  ...
  return {
    tooltip: { trigger: 'axis', valueFormatter: (v: unknown) => typeof v === 'number' ? `${v.toFixed(0)}ms` : '—' },
    legend: { bottom: 0 },
    grid: { left: 70, right: 16, top: 16, bottom: 40 },
    xAxis: { type: 'category', data: labels, axisLabel: { fontSize: 11 } },
    yAxis: { type: 'value', name: 'ms', nameLocation: 'middle', nameGap: 50, axisLabel: { fontSize: 11 } },
    series,
  }
})
...
<template>
  <div class="chart-wrap">
    <h3 class="chart-title">Mean TTFT by model</h3>
    <div ref="chartEl" class="chart-canvas" role="img" aria-label="Mean time to first token line chart" />
  </div>
</template>
